...
class SearchBgHelper(
    ...
    private val secondaryColor: Int = context.attrValue(ru.skillbranch.skillarticles.R.attr.colorSecondary) //colorSecondary
    private val alphaColor: Int = androidx.core.graphics.ColorUtils.setAlphaComponent(secondaryColor, 160)//colorSecondary with 160 alpha

    private val drawable: Drawable by lazy {// для выделения текста с закруглениямии слева и справа
    ...
    private val drawableLeft: Drawable by lazy { // для выделения текста с закруглениямии слева
        mockDrawable ?: GradientDrawable().apply {
            shape = GradientDrawable.RECTANGLE
    ...
    private val drawableMiddle: Drawable by lazy { // для выделения текста без закруглений
        mockDrawable ?: GradientDrawable().apply {
            shape = GradientDrawable.RECTANGLE
            color = ColorStateList.valueOf(alphaColor)
            setStroke(borderWidth, secondaryColor)
    ...
    private val drawableRight: Drawable by lazy { // для выделения текста с закруглениямии справа
        mockDrawable ?: GradientDrawable().apply {
            shape = GradientDrawable.RECTANGLE
            cornerRadii = floatArrayOf(
